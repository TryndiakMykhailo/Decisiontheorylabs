...
def open_file():
    try:
        return open("tryndiaklab4_27.txt")
...
def draw_header(objects, maxWidths, getStartLength = False):
    ...
    return numberPart + parameterPart + '|' + coefficientPart + '|' + Fore.RED + objectsPart + Style.RESET_ALL
...
def print_total (header, objects, parameters, maxWidths):
    header_line = ''
    for i in range(len(header) - len(Style.RESET_ALL) - len(Fore.RED )): header_line += '*'
    ...
    for obj in objTotal:
        if maxValue == obj:
            totalPart += Fore.BLUE  + Back.LIGHTWHITE_EX
    ...
    print(totalPart)
    print(header_line)
    print('Найкращий бренд мобільних телефонів - ' + Fore.BLUE  + Back.LIGHTWHITE_EX + '(' + maxValue + ')' + Style.RESET_ALL)
...
print('\nРезультат значень парамерів кожного бренду:')
for param in parameters:
    for obj in objects:
